...
const mutationDelete = useMutationDeleteDepartment();
...
const options = computed(() => {
    return {
        ...
        ...(querySearch && { "name[like]": querySearch }),
    };
});

const { data, isLoading } = useGetDepartment(options.value);
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({
                queryKey: getQueryKeys({
                    key: queryKeys.department.GET_ALL,
                    ...options.value,
                }),
                exact: true,
            });
    ...
    <v-card class="mx-30 pa-30">
        ...
        >
            ...
            <template v-slot:item.image="{ value }">
                <v-img :src="urlImage(value, 'department')"></v-img>
            </template>

            <template v-slot:item.actions="{ item }">
                <router-link
                    :to="{ name: 'edit_department', params: { id: item.id } }"
                >
                    <v-icon
                        class="me-2"
                        size="small"
